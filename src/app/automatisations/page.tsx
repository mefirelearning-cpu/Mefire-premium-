...
export default async function AutomationsPage(){
 const {rules,jobs,globalPaused}=await listAutomationOverview();
 ...
 return <><div className="top"><div><h1>Automatisations</h1><div className="muted">Contrôle réel des tâches planifiées et des règles enregistrées.</div></div><div className="automation-controls">{globalPaused?<form action={resumeAutomationsAction}><button type="submit">Reprendre toutes les automatisations</button></form>:<form action={pauseAutomationsAction}><button type="submit" className="secondary-button">Pause générale</button></form>}</div></div>
 {globalPaused?<div className="notice" style={{marginTop:18}}><strong>Automatisations en pause.</strong><br/><span className="muted">Le worker n’enverra aucune tâche programmée, y compris celles créées pendant la pause.</span></div>:null}
